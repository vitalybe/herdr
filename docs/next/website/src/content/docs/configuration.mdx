...
If you drive agents mostly from the agents panel, the tab bar and spaces list can get crowded with agent tabs you never switch to directly.

Set `hide_tabs_with_agents = true` to keep agent tabs out of the way:
...
- Tabs that contain a running agent are dropped from the top tab bar. The active tab stays visible even if it is an agent tab, so a focused agent is never hidden.
- Spaces whose tabs are all agents disappear from the spaces list, the collapsed rail, and space navigation.
- Focusing an agent no longer highlights its space; the space selection clears while an agent is focused.

Tabs and spaces that mix agents with plain shells stay visible. You can also toggle this from Settings > Experiments > hide tabs with agents.
