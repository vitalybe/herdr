...
`previous_pane` and `next_pane` cycle focus through the sidebar tabs band, one entry per tab across all spaces. Focusing an entry switches to that tab, exactly like clicking the row. Cycling wraps around and skips line-split dividers. Both are unset by default; bind them with a modified chord such as:
...
`keys.scratch_terminal` (default `ctrl+backtick`) shows a session-modal shell
that keeps running while hidden. It starts in the working directory of the pane
it was opened from, and the same key hides it again and returns focus to that
pane. Reopening restores the same shell with its history and working directory
intact.

`keys.scratch_terminal_to_tab` (default `ctrl+shift+backtick`) moves the scratch
shell into a tab of its own, history included, whether the modal is showing or
hidden. The next `keys.scratch_terminal` then starts a fresh scratch shell.
Exiting the scratch shell also ends it, and the next toggle starts a new one.

```toml
[keys]
scratch_terminal = "ctrl+backtick"
scratch_terminal_to_tab = "ctrl+shift+backtick"
```

Both bindings need a direct chord your terminal can actually send; not every
terminal distinguishes `ctrl+backtick`. Rebind them to any other chord your
setup delivers. A `prefix+` binding also works, but only while the modal is
hidden, because the scratch terminal receives every key until it is dismissed.

The scratch terminal receives all terminal input except its own bindings,
Escape included, so full-screen programs such as editors work in it.

Text selection works as it does in a pane: drag to select, and with
`ui.copy_on_select` the release copies. With `copy_on_select = false` the
selection is retained until Ctrl+C or Cmd+C copies it. A program in the scratch
terminal that reads the mouse itself receives the drag instead, same as in a
pane.

The modal is centred and takes half the terminal in each direction. Size it with
`ui.scratch_terminal_width` and `ui.scratch_terminal_height`, as a number of
cells or a percentage string:

```toml
[ui]
scratch_terminal_width = "80%"
scratch_terminal_height = "80%"
```
...
## Hide agent-only spaces

If you drive agents mostly from the agents panel, the spaces list can get crowded with agent-only spaces you never switch to directly.
...
- Spaces whose tabs are all agent tabs disappear from the spaces list, the collapsed sidebar rail, and space navigation.
- Focusing an agent tab no longer highlights its space; the space selection clears while an agent tab is focused.

A tab counts as an agent tab only when every pane in it is an agent. Tabs that mix agents with plain shells are not agent tabs, and spaces containing any non-agent tab stay visible. Tabs are never removed from the top tab bar.
