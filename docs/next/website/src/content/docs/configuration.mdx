...
it was opened from, and Escape or the same key hides it again and returns focus
to that pane. Reopening restores the same shell with its history and working
directory intact.
...
The scratch terminal receives all terminal input except its own bindings, and
Escape dismisses it rather than reaching the shell, so full-screen programs that
rely on Escape belong in a pane or a popup command instead.
