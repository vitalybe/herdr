...
Agents that record a parent nest under it in the sidebar's agents section. Set the link with `herdr agent set-parent <target> <parent>`, or in manual sort mode by dragging one agent onto another.
