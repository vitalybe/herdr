...
A tab is another terminal layout inside a workspace. Without `--workspace`, `tab create` uses the active workspace and fails if none exists. Its JSON response exposes `.result.tab.tab_id` and `.result.root_pane.pane_id`. `--index N` places the new tab's row at slot `N` in the sidebar Tabs band instead of at the end. The band is one flat list covering every space, and it also holds line-split dividers, so `N` may be anything from `0` up to the current number of rows in it, and anything larger is rejected. `--workspace` still decides which space owns the tab; the slot only places its row, and the tab's position inside that space follows from it, landing just after the nearest row above `N` that belongs to the same space. Every `tab` object carries its current slot as `.index`, in the same terms `--index` takes, so a pane can read its own row's slot with `herdr tab get $HERDR_TAB_ID` and pass `.index + 1` to open a tab directly below itself. That slot moves as tabs around it are created, reordered, or closed, so read it when you need it rather than caching it. Closing a workspace's last tab also closes the workspace, matching the TUI close-tab action. If `confirm_close` is enabled and closing the tab would also close a whole worktree group, `tab close` returns a `confirmation_required` error instead.
