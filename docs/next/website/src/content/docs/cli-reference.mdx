...
A tab is another terminal layout inside a workspace. Without `--workspace`, `tab create` uses the active workspace and fails if none exists. Its JSON response exposes `.result.tab.tab_id` and `.result.root_pane.pane_id`. `--index N` places the new tab's row at slot `N` in the sidebar Tabs band instead of at the end. The band is one flat list covering every space, and it also holds line-split dividers, so `N` may be anything from `0` up to the current number of rows in it, and anything larger is rejected. The slot places the row only; the tab is appended to the space `--workspace` names, exactly as it would be without a slot, so the band order and a space's own tab order are independent. Every `tab` object carries its current slot as `.index`, in the same terms `--index` takes, so a pane can read its own row's slot with `herdr tab get $HERDR_TAB_ID` and pass `.index + 1` to open a tab directly below itself. Because the slot counts every row in the band, the tabs of one space are not numbered consecutively. That slot moves as rows around it are added, reordered, or closed, so read it when you need it rather than caching it. Closing a workspace's last tab also closes the workspace, matching the TUI close-tab action. If `confirm_close` is enabled and closing the tab would also close a whole worktree group, `tab close` returns a `confirmation_required` error instead.
...
`popup close` dismisses the popup terminal currently on screen. The scratch terminal is hidden with its shell left running, exactly as pressing `ctrl+backtick` again does, so reopening it keeps its history; any other popup, such as a plugin popup pane, is closed and its process ends. It returns `popup_not_open` when no popup is visible, including while the scratch terminal is hidden.
