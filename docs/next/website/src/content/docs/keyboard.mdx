...
| Reopen closed tab / workspace | `prefix+u` |
...
Press `prefix+/` to search the focused pane's scrollback. Type your query and matches highlight as you go; the current match count shows in the prompt. Press Enter to confirm, then `n` and `N` to step to the next and previous match. Press Esc to cancel and return to the terminal at your original position. From inside copy mode you can also start a search with `/` (forward) or `?` (backward). Matching is case-insensitive.
