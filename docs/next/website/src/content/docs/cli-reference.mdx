...
`popup close` closes the popup terminal currently on screen, whether it is the scratch terminal or a plugin popup pane, and ends its process. It returns `popup_not_open` when no popup is visible, including while the scratch terminal is hidden.
