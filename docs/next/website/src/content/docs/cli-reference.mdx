...
`agent children [target]` lists the child agents nested under an agent in the agents-panel tree. The target resolves with the same rules and must be an agent pane; when omitted it defaults to the caller's own pane via `$HERDR_PANE_ID`, so the command can be run from inside an agent pane without repeating its id. By default it prints one public pane id per direct child, one per line, so it drops straight into shell loops; add `--recursive` to include the whole descendant subtree in tree (preorder) order, and `--json` to emit the full agent records instead of bare pane ids.
