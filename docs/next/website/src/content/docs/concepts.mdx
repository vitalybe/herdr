...
A tab that holds exactly one agent takes its name from that agent's own session
name, so a Claude Code `/rename` also renames the tab. Renaming the tab in the
Herdr UI pins the name and stops the automatic updates. `herdr tab rename` is
advisory: it names the tab right away, and a later session name supersedes it.
