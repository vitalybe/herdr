...
エージェントを主にエージェントパネルから操作している場合、直接切り替えることのないエージェントのみのスペースでスペース一覧が混み合うことがあります。
...
- すべてのタブがエージェントタブであるスペースは、スペース一覧、折りたたみ時のサイドバーレール、スペースナビゲーションから消えます。
- エージェントタブにフォーカスしてもそのスペースはハイライトされません。エージェントタブにフォーカスしている間はスペースの選択表示が解除されます。

タブがエージェントタブとみなされるのは、その中のすべてのペインがエージェントである場合のみです。エージェントと通常のシェルが混在するタブはエージェントタブではなく、エージェントタブ以外を含むスペースは表示されたままになります。上部のタブバーからタブが消えることはありません。
