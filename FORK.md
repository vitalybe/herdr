...
| Sidebar tabs band | A third band listing every tab across spaces, one row per tab. A tab holding at least one agent pane is an agent tab and renders exactly as the agents band renders that agent, showing the most urgent status when the tab holds several (blocked, then done, then working, then idle). Every other tab takes a fixed neon-green `❯` prompt icon and no status. Rows carry `keys.previous_pane` / `keys.next_pane` cycling, drag-to-reorder, and double-click tab rename. The band is the order the API addresses tabs by: `TabInfo::index` reports a tab's slot in it and `tab.create` takes one as `index`, which pins the new row to that slot and nothing else - the tab is appended to its space either way, so the band and a space's own tab order are independent. A row is placed once, when the tab first appears, and is never regrouped by the space it belongs to afterwards. This is the one place a runtime API reads client-owned presentation state, so the band is reconciled on the API path as well as on render. |
...
| Scratch terminal | A persistent session-modal shell on `keys.scratch_terminal` (`ctrl+backtick`). It rides upstream's popup pane, marked `scratch`: the toggle key parks it in `AppState::hidden_scratch_popup` with its PTY alive instead of closing it, so reopening keeps the history. Every other key reaches the shell, Escape included, so editors work in it. Unlike command popups it keeps its pane identity and agent detection, so `keys.scratch_terminal_to_tab` (`ctrl+shift+backtick`) can hand its terminal to `Workspace::create_tab_from_existing_pane` as a normal pane. Command popups keep upstream's behaviour. Mouse text selection, `ui.copy_on_select`, and the Ctrl+C/Cmd+C copy of a retained selection work in any popup: `handle_popup_mouse` anchors a `Selection` on the popup's own pane id when the popup program is not reading the mouse, `AppState::copy_selection` resolves that pane through the popup runtime, and the selection invariants accept a pane that belongs to no workspace. `ui.scratch_terminal_width` and `ui.scratch_terminal_height` size the modal in cells or percent, defaulting to upstream's half-screen popup geometry. `herdr popup close` exposes upstream's `popup.close` API method to scripts; it closes whichever popup is on screen and ends its process, so on the scratch terminal it discards rather than parks. |
| Tabs named by creation time | A tab nobody has named shows the `hh:mm` it was created (`Tab::created_name`, stamped by the real constructors and carried in the session snapshot) instead of upstream's position number. A rename, a tooling name, or an agent session name still wins, and `Tab::is_numbered` still treats the tab as unnamed, so the tab bar styles it as before. Tabs restored from a snapshot written before this landed fall back to the position number. |
