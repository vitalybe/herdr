...
fn agent_entry_height_in_body(app: &AppState, entry: &AgentPanelEntry, body_height: u16) -> u16 {
    let lines = resolved_agent_rows(app, entry).len().max(1);
    (lines.min(u16::MAX as usize) as u16).min(body_height)
}
...
/// Full ordered list of visible agent-panel rows. Pure.
fn agent_panel_rows_with_runtimes(
    app: &AppState,
    terminal_runtimes: Option<&TerminalRuntimeRegistry>,
) -> Vec<AgentPanelRow> {
    let base = base_agent_panel_rows(app, terminal_runtimes);
    apply_agent_line_split_collapse(app, base)
}
...
/// The flat, sort-driven ordering of agent rows.
...
fn render_token_rows(
    ...
    for (row_index, resolved) in rows.iter().take(content_rows).enumerate() {
        ...
        let used = if row_index == 0 { 1 } else { 3 };
        spans.extend(resolved_token_spans(
            resolved,
            state_icon,
            status_style,
