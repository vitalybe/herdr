...
如果你主要通过智能体面板来操作智能体，空间列表可能会被那些你从不直接切换过去的仅含智能体的空间挤满。
...
- 所有标签页都是智能体标签页的空间会从空间列表、折叠后的侧栏轨道以及空间导航中消失。
- 聚焦智能体标签页不再高亮其所属空间；只要聚焦的是智能体标签页，空间选中状态就会清除。

只有当标签页中的每个窗格都是智能体时，该标签页才算智能体标签页。混合了智能体与普通 shell 的标签页不是智能体标签页，包含任何非智能体标签页的空间会保持可见。标签页永远不会从顶部标签栏中移除。
